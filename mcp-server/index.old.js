...
const TOOLS = [
  ...
  // ============================================
  // NEW BATCH 2 TOOLS - Database, API, Messaging
  // ============================================

  // Database Query Tool
  queryDatabaseTool.toolDefinition,
  queryDatabaseTool.listTablesDefinition,
  queryDatabaseTool.testConnectionDefinition,

  // ServiceTitan API Tool
  callStApiTool.toolDefinition,

  // SMS Tool
  sendSmsTool.toolDefinition,
  sendSmsTool.getMessageStatusDefinition,

  // Email Tool
  sendEmailTool.toolDefinition,
  sendEmailTool.sendBulkEmailDefinition,

  // Create Job Tool
  createJobTool.toolDefinition,
  createJobTool.getJobTypesDefinition,
  createJobTool.getBusinessUnitsDefinition,

  // Schedule Appointment Tool
  scheduleAppointmentTool.toolDefinition,
  scheduleAppointmentTool.getAvailabilityDefinition,
  scheduleAppointmentTool.getAppointmentsDefinition,
  scheduleAppointmentTool.rescheduleDefinition,
  scheduleAppointmentTool.getTechniciansDefinition,
];
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      ...
      // ============================================
      // NEW BATCH 2 TOOL HANDLERS
      // ============================================

      // Database Tools
      case 'query_database':
        result = await queryDatabaseTool.queryDatabase(args.query, args.params || []);
        break;

      case 'list_database_tables':
        result = await queryDatabaseTool.listTables();
        break;

      case 'test_database_connection':
        result = await queryDatabaseTool.testConnection();
        break;

      // ServiceTitan API Tool
      case 'call_st_api':
        result = await callStApiTool.handleToolCall(args);
        break;

      // SMS Tools
      case 'send_sms':
        result = await sendSmsTool.handleToolCall(args);
        break;

      case 'get_sms_status':
        result = await sendSmsTool.getMessageStatus(args.messageId);
        break;

      // Email Tools
      case 'send_email':
        result = await sendEmailTool.handleToolCall(args);
        break;

      case 'send_bulk_email':
        result = await sendEmailTool.sendBulkEmail(args.recipients, args.subject, args.body);
        break;

      // Create Job Tools
      case 'create_job':
        result = await createJobTool.handleToolCall(args);
        break;

      case 'get_job_types':
        result = await createJobTool.getJobTypes();
        break;

      case 'get_business_units':
        result = await createJobTool.getBusinessUnits();
        break;

      // Schedule Appointment Tools
      case 'schedule_appointment':
        result = await scheduleAppointmentTool.handleToolCall(args);
        break;

      case 'get_availability':
        result = await scheduleAppointmentTool.getAvailability(args);
        break;

      case 'get_appointments':
        result = await scheduleAppointmentTool.getAppointments(args);
        break;

      case 'reschedule_appointment':
        result = await scheduleAppointmentTool.rescheduleAppointment(args.appointmentId, args);
        break;

      case 'get_technicians':
        result = await scheduleAppointmentTool.getTechnicians(args);
        break;
