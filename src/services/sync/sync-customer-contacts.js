...
async function fullSyncContacts() {
  ...
  try {
    const stateResult = await client.query(
      "SELECT value FROM servicetitan.sync_state WHERE key = 'contacts_export_token'"
    );
...
async function incrementalSyncContacts() {
  ...
  try {
    const stateResult = await client.query(
      "SELECT value FROM servicetitan.sync_state WHERE key = 'contacts_last_sync'"
    );
    if (stateResult.rows.length > 0) {
...
async function saveSyncState(key, value) {
  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS servicetitan.sync_state (
    ...
    await client.query(`
      INSERT INTO servicetitan.sync_state (key, value, updated_at)
      VALUES ($1, $2, NOW())
